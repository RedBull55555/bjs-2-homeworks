class PrintEditionItem{
	constructor(name, releaseDate, pagesCount){
	 ...
	 fix() {
		this.state *= 1.5;
		if (this.state > 100) {
		  this.state = 100;  // Не позволяем состоянию превышать 100
		}
	  }
...
const sherlock = new PrintEditionItem(
   ...
   const book = new PrintEditionItem('Some Book', 2021, 300);
   console.log(book.state);   // 100
   
   book.state = 50;           // Издание испортилось
   book.fix();
   console.log(book.state);   // 75
   
   book.fix();
   console.log(book.state);   // Максимум 100
